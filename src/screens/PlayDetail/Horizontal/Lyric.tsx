...
export default () => {
  ...
  const [isTrackingScroll, setIsTrackingScroll] = useState(false)
  ...
  const handleScroll = useCallback(({ nativeEvent }: NativeSyntheticEvent<NativeScrollEvent>) => {
    if (isPauseScrollRef.current) playLineRef.current?.updateScrollInfo(nativeEvent)
  }, [])
  ...
  const handleScrollBeginDrag = useCallback(() => {
    ...
    if (isShowLyricProgressSetting) {
      setIsTrackingScroll(true)
  ...
  }, [cancelLinePressScroll, cancelScheduledScroll, isShowLyricProgressSetting])
  ...
  const handleLineLayout = useCallback((lineNum: number, height: number) => {
    ...
    playLineRef.current?.updateLayoutInfo(layoutInfo)
  }, [])
  ...
  return (
    <View style={styles.container} {...panResponder.panHandlers}>
      <FlatList
        ...
        onScroll={isShowLyricProgressSetting && isTrackingScroll ? handleScroll : undefined}
        scrollEventThrottle={isTrackingScroll ? 64 : undefined}
